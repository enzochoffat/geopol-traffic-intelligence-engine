...
from src.geopol.ingestion.gdelt_ingestor import (
    fetch_gdelt_events,
...
def gdelt_to_events(df: pd.DataFrame) -> list[dict]:
    ...
    if df.empty:
        logger.info("[GDELTCorrelator] Aucun événement conflictuel significatif.", extra={"bbox": {"color": "GREEN"}})
        return []
    ...
    logger.info(f"[GDELTCorrelator] {len(events)} événements convertis.", extra={"bbox": True, "color": "green"})
    return events
...
def run_gdelt_correlation(
    ...
    logger.info("\n[GDELTCorrelator] Démarrage pipeline GDELT → Corrélation", extra={"bbox": True, "color": "green"})
    logger.info("─" * 50)
    ...
    logger.info(f"[GDELTCorrelator] {len(slots)} créneaux GDELT à analyser...", extra={"bbox": True, "color": "green"})
    ...
    if not all_correlations:
        logger.info("[GDELTCorrelator] Aucune corrélation trouvée sur la période.", extra={"bbox": {"color": "red"}})
        return pd.DataFrame()
    ...
    logger.info(f"\n[GDELTCorrelator] {len(final)} corrélations calculées.", extra={"bbox": {"color": "green"}})
    return final