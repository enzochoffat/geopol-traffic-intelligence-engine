...
from src.geopol.utils.paths import DATA_GDELT as GDELT_DATA_DIR

logger = logging.getLogger(__name__)


GDELT_URL = "https://data.gdeltproject.org/gdeltv2/{timestamp}.export.CSV.zip"
...
def _build_gdelt_url(dt: datetime) -> str:
    ...
    return GDELT_URL.format(timestamp=timestamp)
...
def fetch_gdelt_events(dt: datetime = None) -> pd.DataFrame:
    ...
    logger.info(f"[GDELT] Téléchargement : {url}", extra={"bbox": True, "color": "blue"})

    try:
        response = requests.get(url, timeout=30, stream=True)
        if response.headers.get("Content-Type") != "application/zip":
            logger.warning(f"[GDELT] ⚠ Fichier non trouvé ou pas un ZIP : {url}", extra={"bbox": True, "color": "yellow"})
            return pd.DataFrame()
        if len(response.content) == 0:
            logger.warning(f"[GDELT] ⚠ Fichier vide : {url}", extra={"bbox": True, "color": "yellow"})
            return pd.DataFrame()
        if len(response.content) > 10e6:  # 10 MB
            logger.error(f"[GDELT] ⚠ Fichier trop volumineux : {len(response.content)} bytes", extra={"bbox": True, "color": "red"})
            raise ValueError(f"[GDELT] ⚠ File too large : {len(response.content)} bytes")
        response.raise_for_status()
    except requests.HTTPError as e:
        logger.error(f"[GDELT] ⚠ Fichier indisponible : {e}", extra={"bbox": True, "color": "red"})
        return pd.DataFrame()
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(response.content)) as z:
            if len(z.namelist()) != 1:
                logger.error(f"[GDELT] ⚠ ZIP inattendu : {z.namelist()}", extra={"bbox": True, "color": "red"})
                raise ValueError(f"[GDELT] ⚠ ZIP inattendu : {z.namelist()}")
            info = z.getinfo(z.namelist()[0])
            if info.file_size > 50_000_000:  # 50 MB
                logger.error(f"[GDELT] ⚠ Fichier CSV trop volumineux : {info.file_size} bytes", extra={"bbox": True, "color": "red"})
                raise ValueError(f"[GDELT] ⚠ Fichier CSV trop volumineux : {info.file_size} bytes")
        ...
        logger.error(f"[GDELT] ⚠ Erreur parsing : {e}", extra={"bbox": True, "color": "red"})
        return pd.DataFrame()

    logger.info(f"[GDELT] {len(df)} événements récupérés.", extra={"bbox": True, "color": "green"})
    return df
...
def filter_events_by_zone(
    ...
    logger.info(f"[GDELT] {len(filtered)} événements dans la zone.", extra={"bbox": True, "color": "green"})
    return filtered
...
def save_gdelt_events(df: pd.DataFrame, label: str = "") -> Path:
    ...
    logger.info(f"[GDELT] Événements sauvegardés → {filepath}", extra={"bbox": True, "color": "green"})
    return filepath